/*OWSSTDBAGO202.ServSuite... Test2 Environment Scripts Combined*/
/*OWSSTDBAGO202 = Live*/
/*OWSSDDBO244 = Replica1*/
...
PRINT '';
PRINT 'Database Refresh Script Last UpdateC: 2019-05-02 by Ben Shearouse';

/*********************************************************************************************************/
/*********************************************************************************************************/
/*********************************************************************************************************/
:Connect OWSSTDBAGO202

PRINT 'Connected to OWSSTDBAGO202';
PRINT GETDATE();
PRINT '';

PRINT 'Beginning Test2 Database Refresh on OWSSTDBAGO202';
PRINT GETDATE();
PRINT '';
...
PRINT 'Removed 5 databases from AlwaysOn Availability Groups';


PRINT 'Disable SQL Agent Job "DatabaseBackup - USER_DATABASES - LOG" on OWSSTDBAGO202';
PRINT 'Disable SQL Agent Job "SQL Backup - DDBoost - TLOG" on OWSSTDBAGO202';
PRINT GETDATE();
PRINT '';
USE msdb ;
...
EXEC dbo.sp_update_job
    @job_name = N'SQL Backup - DDBoost - TLOG',
    @enabled = 0 ;
...
/*Complete Backup Set*/
--SET @backupLocationServSuiteData = N'\\rwfilesvro202\RWBDBAGH201_BOSS_Full\RWBDBAGH201_ProdServSuiteData.full.bak'
--SET @backupLocationServSuiteLog = N'\\rwfilesvro202\RWBDBAGH201_BOSS_Full\RWBDBAGH201_prodServSuiteLog.full.bak'
--SET @backupLocationServSuiteWBProgram = N'\\rwfilesvro202\RWBDBAGH201_BOSS_Full\RWBDBAGH201_ProdServSuiteWBProgram.full.bak'
--SET @backupLocationServSuiteWBReport = N'\\rwfilesvro202\RWBDBAGH201_BOSS_Full\RWBDBAGH201_ProdServSuiteWBReport.full.bak'
--SET @backupLocationServSuiteWBUser = N'\\rwfilesvro202\RWBDBAGH201_BOSS_Full\RWBDBAGH201_ProdServSuiteWBUser.full.bak'
...
PRINT '';
PRINT '';
PRINT 'Restore Integrated Test Databases on OWSSTDBAGO202';
PRINT GETDATE();
...
PRINT 'Connected to OWSSDDBO244';
PRINT GETDATE();
PRINT '';
...
PRINT '';
PRINT '';
PRINT 'Replica1 SQL Server Database Refresh on OWSSDDBO244';
PRINT GETDATE();
...
PRINT 'Recover Live databases on OWSSTDBAGO202';
PRINT GETDATE();
PRINT '';
...
PRINT 'Join AlwaysOn databases to BOSS_AG1 on OWSSDDBO244';
PRINT GETDATE();
PRINT '';
...
PRINT 'Enabled SQL Agent Job "SQL Backup - DDBoost - TLOG" on OWSSTDBAGO201';
PRINT 'Leave SQL Agent Job "DatabaseBackup - USER_DATABASES - LOG" disabled on OWSSTDBAGO201';
PRINT GETDATE();
PRINT '';
USE [msdb];
EXEC dbo.sp_update_job
...
IF EXISTS (select * from sys.sysusers where name = 'ROLLINS\ServSuite Orkin Prod Read-Only') DROP USER [ROLLINS\ServSuite Orkin Prod Read-Only];
...
IF EXISTS (select * from sys.sysusers where name = 'ROLLINS\SSPRDSVC') DROP USER [ROLLINS\SSPRDSVC];
IF EXISTS (select * from sys.sysusers where name = 'ROLLINS\ServSuite Orkin BI Prod Read-Only') DROP USER [ROLLINS\ServSuite Orkin BI Prod Read-Only];
IF EXISTS (select * from sys.sysusers where name = 'ROLLINS\ServSuite Orkin SSIS Prod Read-Only') DROP USER [ROLLINS\ServSuite Orkin SSIS Prod Read-Only];
IF EXISTS (select * from sys.sysusers where name = 'ROLLINS\SVC_AppDyn') DROP USER [ROLLINS\SVC_AppDyn];
IF EXISTS (select * from sys.sysusers where name = 'ROLLINS\SVC_OSP_Admin_Prod') DROP USER [ROLLINS\SVC_OSP_Admin_Prod];
IF EXISTS (select * from sys.sysusers where name = 'servsuiteWM') DROP USER [servsuiteWM];
IF EXISTS (select * from sys.sysusers where name = 'servsuitePI') DROP USER [servsuitePI];
IF EXISTS (select * from sys.sysusers where name = 'servsuiteapp') DROP USER [servsuiteapp];
IF EXISTS (select * from sys.sysusers where name = 'servsuiteJDE') DROP USER [servsuiteJDE];
IF EXISTS (select * from sys.sysusers where name = 'servsuiteSVC') DROP USER [servsuiteSVC];
IF EXISTS (select * from sys.sysusers where name = 'servsuiteVRM') DROP USER [servsuiteVRM];
IF EXISTS (select * from sys.sysusers where name = 'servsuiteCWR') DROP USER [servsuiteCWR];
IF EXISTS (select * from sys.sysusers where name = 'servsuitedeploy') DROP USER [servsuitedeploy];
...
USE ServSuiteWBProgram;
UPDATE tlinks
SET linkurl = 'https://Test2-servsuite.rollins.com/fileserver/fileupload.aspx'
WHERE linkid = 339;
...
/*this part is only for Dev2 and Test2 per Troy.*/
--turn on routes belonging to the 10
USE [ServSuiteData];
SELECT routeid INTO #tmp FROM tbranchroute br
INNER JOIN tbranch b ON br.branchid = b.branchid 
WHERE glprefix in ('165');

UPDATE troute
SET dailyoptimization = 1 
WHERE routeid IN (SELECT routeid FROM #tmp);
...
--Removed Round 1 deletes because they have moved to production
*/
...
/**************************************************************************************************************************************/
/*Update synonyms in ServSuiteData db to point to ServSuiteLog if it is pointing to prodServSuiteLog.                     */
/*Added by Ben Shearouse on 12/21/2020.                                                                                                 */
/*https://www.sqlservercentral.com/blogs/drop-and-recreate-all-synonyms                                                        */
/**************************************************************************************************************************************/

PRINT'';
PRINT'';
PRINT 'Update synonyms in ServSuiteData db to point to ServSuiteLog if it is pointing to prodServSuiteLog';
PRINT GETDATE();
USE ServSuiteData;
...
BEGIN
    ...
    SET @SQL = 'CREATE SYNONYM ' + QUOTENAME(@Schema) + '.' + QUOTENAME(@ObjectName) + ' FOR ' +
                  REPLACE(@Definition, '[prodServSuiteLog].', '[ServSuiteLog].')
    EXEC(@SQL)
...
PRINT '';
PRINT '';
PRINT 'Test2 Environment Database Refresh Complete on OWSSTDBAGO202';
PRINT 'and AlwaysOn set up on OWSSDDBO244';
PRINT GETDATE();
/*********************************************************************************************************/
/*********************************************************************************************************/
/*********************************************************************************************************/
