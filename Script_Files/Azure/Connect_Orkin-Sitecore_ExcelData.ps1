...
# read-host -Prompt 'Enter Password' -assecurestring | convertfrom-securestring | out-file C:\PowerSh\Rollins\DBA_Admin\Script_Files\Azure\AzureConnPass.txt;     *****a32
...
$Results = invoke-sqlcmd -ServerInstance RWBDBAGH213\MOBILE01 -Database prodServSuiteData -InputFile "C:\PowerSh\Rollins\DBA_Admin\Script_Files\Azure\Select_tWebCommerceBasket.sql" -As DataTables;
...
<#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------#>
# Connect to BOSS production Replica2, execute SQL query ecomm_sales_lineitems.sql and store results in new tab in Excel file. Windows Authentication used when user and password not supplied.
write-host "Executing eCommerce leads query from Replica2 data .... " -ForegroundColor Green;
$TableR2 = Invoke-Sqlcmd -InputFile "C:\PowerSh\Rollins\DBA_Admin\Script_Files\Azure\ecomm_sales_lineitems.sql" -ServerInstance RWBDBAGH213\MOBILE01 -Database prodServSuiteData -AbortOnError;
...
write-host "Executing eCommerce leads query from Replica2 data .... " -ForegroundColor Green;
$TableR2 = Invoke-Sqlcmd -InputFile "C:\PowerSh\Rollins\DBA_Admin\Script_Files\Azure\ecomm_leads.sql" -ServerInstance RWBDBAGH213\MOBILE01 -Database prodServSuiteData -AbortOnError;
$TableR2 | Export-Csv "C:\PowerSh\Rollins\DBA_Admin\Script_Files\Azure\eComm3.csv" -NoTypeInformation -UseCulture -force;

<#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------#>
<#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------#>
# Merge Excel .CSV files - sales: eComm1.csv & leads: eComm2.csv & sales lineitems: eComm3.csv into eComm_data.xlsx. 
# Rename each worksheet as Sales, Leads, Sales - LineItems in file eComm_data.xlsx.
# File names eComm1, eComm2, eComm3 is named due to sorting requirement when merging the two files.
...
$Workbook.Worksheets.Item(2).Name = "Sales - LineItems";
$Workbook.Worksheets.Item(3).Name = "Leads";
