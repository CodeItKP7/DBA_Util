...
$srcdb = 'DBA'
$srcTab = 'TransactionHistoryTEST'


#GET THE LIST OF SQL SERVERS
$Global:srvList = import-csv -path 'C:\PowerSh\Rollins\DBA_Admin\TxServers.csv'
...
FOREACH($instance in $Global:srvList.instance)
    ...
    $SQL = "select (Convert(varchar,sqlserver_start_time,20)-1) as [SrvStart] from SYS.dm_os_sys_info"

    $srvStartTime = Invoke-Sqlcmd -ServerInstance $instance -Query $SQL    

#write-host $srvStartTime.SrvStart
...
#write-host $RecordedStartUp.CurrentStartup
